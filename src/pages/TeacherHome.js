...
import Notifications from "../components/Notifications"; // optional if you want notifications for teacher
import { db, auth } from "../firebase/firebaseConfig"; // <-- import auth
import { sendNotification } from "../firebase/notificationHelpers"; // Add this import
import {
  markAttendance,
  deleteLecture,
...
export default function TeacherHome() {
  ...
  const handleMark = async (studentId, status) => {
    ...
    if (prev.todaysStatus === status) {
      ...
      if (prev.todaysStatus === "present") attended = Math.max(attended - 1, 0);
      const percentage = total === 0 ? "0.00" : ((attended / total) * 100).toFixed(2);
    ...
    if (!prev.todaysStatus) {
      ...
      if (prev.todaysStatus === "present" && status === "absent") attended -= 1;
      else if (prev.todaysStatus === "absent" && status === "present") attended += 1;
    }
    ...
    studentId,
    "Attendance Updated",
    `Your attendance for ${selectedSubject} on ${selectedDate} is marked as ${status.toUpperCase()}`
  );
  };
...
{/* Filters */}
<div key={loading ? "loading" : "ready"} className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
  <div>
    <label className="block mb-1 font-semibold text-white">Subject:</label>
    <select
  ...
  <div>
    ...
    />
        ...
        {/* Notifications for Teacher (Optional) */}
        <Notifications userId={auth.currentUser.uid} role="teacher" />
